...
from app.integrations.heatwave_explanations import risk_feature_fingerprint
...
def explanation(row, native=None):
    conn = MagicMock()
    adapter = HeatWaveMLAdapter(conn)
    source_row = {**row, "model_trained_at": "2026-09-23 00:08:00"}
    persisted = (
        [
            {
                "ml_results": json.dumps(native),
    ...
    adapter._safe_query = MagicMock(side_effect=[[source_row], persisted])
    return adapter.explain_risk(1), conn


def test_zero_success_rates_reach_rule_explanation():
...
def test_no_positive_factors_do_not_create_a_hundred_percent_cause(native):
    result, _ = explanation(
        {"construction_pct": 100, "integration_success_pct": 100, "progress_slope_14d": 3}, native
    )
    ...
    conn.execute.return_value.mappings.return_value.all.return_value = [
        {
            ...
            "model": "model",
            "source": "llm",
            "generated_at": "2026-09-12 16:30:00",
        }
    ]
...
def test_fit_scores_do_not_certify_business_forecasts(monkeypatch):
    ...
    hw = MagicMock()
    hw.get_status.return_value = {
        "status": "ready",
        "models": {"regression": {"quality": 0.99}, "classifier": {"quality": 0.9}},
    }
