...
describe('decision panel options', () => {
  it('keeps each decision fact in one visible panel', () => {
    ...
    expect(sources.operations).toContain('title="审批与制证流转"')
    expect(sources.operations).toContain('zone="D7"')
    expect(sources.operations).toContain('zone="D6" subtitle="覆盖规模与稽核状态 · 未核验项明确标注" class="col-span-9"')
    expect(sources.operations).toContain('class="col-span-7 pr-3 border-r border-surface-veil-06"')
    expect(sources.operations).toContain('zone="D7" subtitle="新数据近 30 日窗口 · 历史存量不回算" class="col-span-3"')
    expect(sources.theme).toContain('--grid-template-columns-operations: repeat(12, minmax(0, 1fr))')
    expect(sources.operations).not.toContain('zone="D6" subtitle="覆盖规模与稽核状态 · 未核验项明确标注" class="col-span-2"')
    ...
    expect(sources.dashboard).toContain('`AI 日报 · ${briefing.briefingDate}`')
