...
describe('decision panel options', () => {
  it('keeps each decision fact in one visible panel', () => {
    ...
    expect(sources.operations).toContain('zone="D6" subtitle="覆盖规模与稽核状态 · 未核验项明确标注" class="col-span-7"')
    expect(sources.operations).toContain('zone="D7" subtitle="新数据近 30 日窗口 · 历史存量不回算" class="col-span-5"')
    ...
    expect(sources.dashboard).toContain('`AI 简报 · 截至 ${briefingAsOf}`')
