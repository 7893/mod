...
- 状态：IN-PROGRESS（2026-09-05 立项，派 agy 执行，主控验收）
...
- 范围与顺序（任务书内部分阶段，逐阶段独立提交+独立验证）：A 建设事件足迹模型+校验（不写库）→
  B 7 类剧本生成器（dry-run 可核）→ C B模式推进器（状态机核心）→ D 快电影喂慢电影+矛盾咬合 →
  E dry-run 全量自检 + 授权后真实写库 + 事实文档更新。
- 写库纪律：默认关闭；每写库阶段先备份将改动的表、先 dry-run、分批（2000~10000 行/批、逐批 commit、
  打印进度）、由主控逐阶段授权后才真实写入；零 schema 变更；只增不删；接续存量时间线；人员只从现有名录选。
- 验收（8 条硬闸门，主控实测）：①四级下钻加总一致 ②阶段跃迁只进不退 ③跃迁有过程有留痕
  ④横截面自洽（无未上线却有正式业务、双轨达标却显示未上线、状态/时间倒流）⑤运行后仍满足 KI-017
  ⑥接续存量时间线无倒插 ⑦困难户与风险预测一致 ⑧make check 绿、写库分批有进度日志、每阶段有备份。
- 分工：设计与验收由主控 agent 负责；实现由 agy 执行（脚本 `scripts/agy/`）。派工单见
  `scripts/kiro/tmp/mod-tasks-for-agy-step2.html`（便捷传达，长期留档以本 KI 与设计规范为准）。
- 关联：KI-017（存量治理，本步不得破坏其成果）、KI-023/KI-015（决策支撑真实性，矛盾咬合的盾侧）。
