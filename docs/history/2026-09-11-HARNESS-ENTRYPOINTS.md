...
更新日期：2026-09-11
状态：历史
适用范围：通用本地 harness 接入前的 MOD 自动加载入口保全

原始位置：下列各节标明。取代原因：入口改为按领域读取，原文完整保留供追溯，不再默认注入会话。
现行说明：[本地 Harness](../development/LOCAL-HARNESS.md)。
...
Antigravity/agy-specific temporary or diagnostic scripts must be placed under `scripts/agy/`. Do not write
scripts into the repository root, another CLI's directory, `database/`, `generator/`, or `tools/`.

```

## 原始文件：.github/copilot-instructions.md
...
## 原始文件：.kiro/steering/mod-project-rules.md
