...
- B 屏已删除 B2 同源雷达，B3 改为滞后视角，B6 台账预览常驻；完整筛选与调态台账使用宽抽屉，不再替换主画布。C 屏已把批次当前构成与历史爬坡合并到 C2，省域改为推进缺口视角，C5 仅在联系人覆盖存在例外时显示分布。E/F 暂保留第一轮面板结构，下一阶段去除合规构成、风险与模型质量重复。时间序列或异常字段缺失时必须显示明确空态。
