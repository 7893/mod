...
- 六屏路由与领域结构沿用[历史六屏规范](../history/23-V2六屏驾驶舱页面区域与布局规范.md)，
  Zone 编号按当前信息架构连续维护，且**不沿用其中的具体数据数值与旧 CSS 标记规则**：
  - 历史规范内写死的行数、单位数、指标数值均为早期封版快照，已过时，以数据库只读结果为准。
  - 历史规范第四节的 `class="panel"`、`zone-badge`、`#00d2ff 青色发光`等旧 CSS 标记规则**已废除**，
...
- 面板事实单一归属与主画布全量可见由
  [ADR-0017](../decisions/0017-六屏面板事实单一归属与主画布全量可见.md)记录；当前 Zone 连续编号规则由
  [ADR-0019](../decisions/0019-六屏主面板采用连续编号.md)取代 ADR-0017 的旧编号保留条款。
...
Zone 编号是当前产品坐标，用于沟通定位，不得替代业务标题，也不绑定具体 CSS 实现。每屏 N 个主面板
必须连续编号为 `{屏名}1 … {屏名}N`，不得有空号或重复号；信息架构变更可以合并或退役低价值面板，
但必须在同一变更中重排后续编号并同步代码、现行文档和测试。历史文档保留当时编号，不追溯改写；
抽屉或详情页可使用 `B6-DETAIL` 一类来源后缀，不计入主画布序列。
...
- 六屏均已使用 `CockpitPanel` 外壳与具名 Grid Token。2026-09-09 完成 C/D/E/F 屏积木化：首行指挥带统一为 `blocks/CommandBand`（左主图 + 右事实栏），面板内事实栏/对账明细/TOP 列表统一为 `MetricGrid`/`StatList` 的 `flat` 平铺形态，规则告警统一为 `StatusList`，提示条统一为 `NoteBanner`，空态统一为 `EmptyNote`；当前 D6 金标准核验四卡收敛为 `MetricGrid` 数据映射。
- 去「模板感」原则：面板内不再叠彩色底卡（card-in-panel），分区只用 hairline 分隔；没有语义的装饰线（如未稽核项的空进度轨）、胶囊标签一律不画；文本内容宁可滚动也不得截断成「另有 N 条」。2026-09-09 据此改写 F5 简报（`BriefingList` 纵向全文）、当前 D6 核验卡（去进度轨）、F3 分布图（去底卡、隐藏与柱标重叠的坐标刻度）。
...
- `ChartFacts` 仅提供 `chart-led`、`facts-led` 两种组合，负责图表/事实栏的比例、分隔和事实滚动；D3/D4/D5 复用它，不在页面重新定义同构比例。
