...
- JPA `/home/ubuntu/mod`：唯一源码、Git、文档、测试、工具、数据和历史资产工作区，不承载 MOD 生产服务。
- USA `/home/ubuntu/mod`：纯生产 release、`current` 软链、运行环境与部署配置，不初始化 Git，
  不保留测试、历史、原始数据或开发工具（见 ADR-0012）。
- 环境文件只留在实际使用它的主机，永远不进入 Git 或历史归档。
...
- `simulation/`：现行拟真引擎领域与运行时；生产由统一 `mod.service` 托管，不得绕过安全门禁启动。
