...
1. **无数据库离线模式（默认/展示）**：
   - 不需要配置任何数据库；
   - 后端自动加载并提供已验证的内置快照（`app/v2-sim-snapshot.json`），前端直接内置 `fallback-snapshot.json`；
   - 支持六屏业务联动与只读 API 演示；仓库不分发中国地图几何，未配置地图源时地图区域显示明确空态；
   - 部署者确认外部地图的授权、现势性与适用法规后，可用 `VITE_CHINA_MAP_GEOJSON_URL` 指向包含
     `properties.name` 的 GeoJSON `FeatureCollection`，恢复省域地图渲染与下钻。
