...
4. **保全边界必须如实声明**：
   - 项目级 R2 备份与恢复工具已按 KI-095、ADR-0016 退役，不再提供自动异机备份或“主机损毁后百分之百还原”的承诺。
   - 仓库可见的脱敏副本由 GitHub 与 Git 历史保全；本机受限原件只由本机文件和
     `docs/history/MANIFEST.sha256` 校验完整性。若主机及其平台级存储同时丢失，受限原件可能无法恢复。
   - 哈希只能证明已有文件未被改写，不能替代备份。新增受限原件时必须在目录中明确记录其实际保全位置，
     不得把计划中的外部副本写成已经存在。
