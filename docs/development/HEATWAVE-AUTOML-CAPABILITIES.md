...
## 2026-09-23 修正 · KI-107 预生成 SHAP 边界

为保持公网 API 数据库身份的业务 schema 只读权限，仓库已改为由每日管理员重训任务预生成 SHAP：
对超过十列的风险特征表，`ML_EXPLAIN_TABLE` 每批最多 10 行；所有批次进入隔离候选表，行数与
`attributions` 全量校验后才原子切换，同时保留上一版。API 只查询 `mod.ml_risk_explanation`，
快照不可用时标记 `RULE_BASED`。禁止在前台请求内同步调用 `ML_EXPLAIN_ROW`或 `ML_EXPLAIN_TABLE`，
也禁止为此扩大 API 账号的 `sys` 权限。

生产首轮 DDL 与生成尚需单独通过数据库变更闸门；在发布前，下述 2026-09-21 段落仍是当前生产行为记录。

## 2026-09-21 修正 · 在线解释与重训协调

现行单行风险解释通过显式 JSON 类型调用 `ML_EXPLAIN_ROW`，语句最长执行 10 秒，成功 SHAP 结果按输入
特征缓存 15 分钟；失败时返回有明确来源标记的规则解释。禁止在前台请求内用 `ML_EXPLAIN_TABLE` 为全部单位同步生成
解释。每日重训通过 `mod_ml_retrain` advisory lock 串行化，大屏快照刷新发现该锁时延后计算并继续提供
上一份缓存，避免将训练资源竞争误报为数据库故障。

## 2026-09-13 修正 · KI-080

以下旧文中关于“时序预测”“延期预测”“可信泛化”“待训练/已就绪”的概括不能代表当前能力或运行状态，原文保留供追溯。当前训练 SQL 使用同截面公式生成标签，回归按单位上线天数排序切分，分类随机切分单位；评估仅测量合成标签拟合，不能验证未来结果。API 与 F 屏统一显示合成标签实验并保留原始分数，风险名单由业务规则决定。

缺失或非法概率返回 `null` 并附 `probabilitySource`；成功率 0 保留；无正向因素返回空解释。Top 3 百分比仅为所选正向贡献的相对占比，不是风险概率或因果比例。

每日简报仅以白名单宏观数字生成摘要，不包含 AutoML 预测，原因与建议需要另行核实。读取返回 `isStale`/`freshness`，按统一展示时区比较日期，A/F 屏展示旧简报提示。Cloudflare 摘要适配器的每日计数为进程内请求尝试数（失败也计数、重启重置），不是账户预算或持久化配额；与模拟器独立配额账本不同。

本次仅本地代码与离线测试，不执行重训、评分、数据库变更、外部模型调用或部署。验收与后续真实预测边界见 [KI-080](../issues/KI-080-AI-OUTPUT-TRUST.md)。
...
- 离线训练与独立切分验证脚本：[`scripts/agy/train_and_evaluate_models.py`](../../scripts/agy/train_and_evaluate_models.py)
- 只读基线核查入口：[`scripts/agy/run_ml_retrain.py`](../../scripts/agy/run_ml_retrain.py) 的 `--dry-run`
...
1. **“自评分 100%”的虚假繁荣（KI-015）**：
   ...
   - *教训*：训练集自评分存在严重的自拟合假象。在通过 `scripts/agy/train_and_evaluate_models.py` 建立 80%/20% 严谨的独立 Train/Test Split 评估机制后，才真正暴露出模型的泛化问题。**永远不能拿训练集自评分当模型指标对外汇报**。
...
3. **只读核查在先**：每次训练前必须先运行 `scripts/agy/run_ml_retrain.py --dry-run`，核验特征完整性与数据泄露风险；训练结果由同一现行流水线记录独立测试集指标。
...
- [`backend/app/integrations/heatwave_ml.py`](../../backend/app/integrations/heatwave_ml.py) 与 [`backend/app/integrations/cloudflare_ai.py`](../../backend/app/integrations/cloudflare_ai.py)：大屏上层直接使用的 ML 与生成式 AI 适配器，含有效性阈值和安全降级。
- [`scripts/agy/train_and_evaluate_models.py`](../../scripts/agy/train_and_evaluate_models.py)：独立 Train/Test 切分、特征完整性检查、训练与评估全流程工具。
- [`scripts/agy/run_ml_retrain.py`](../../scripts/agy/run_ml_retrain.py)：日常重训入口；`--dry-run` 提供只读基线核查。
