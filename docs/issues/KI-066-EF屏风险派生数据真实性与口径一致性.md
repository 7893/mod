...
- [x] E/F 屏票据类/双轨类风险可依据真实凭证率出现与自愈；不再因 `voucherRate` 恒 `None` 而永不触发。
- [x] A8/D7 的金标异常为真实核验结果或如实标注未核验，不再以硬编码 0 冒充已通过。
- [x] E 屏数据口径统一或分工明确，抽屉/广播与主体表格无误导性不一致。
- [x] A1「本次集成」口径清晰、不与真库当日增量混淆。
- [x] 相关行为变更补充/更新回归测试；`docs/CURRENT-STATE.md` 同步；`make check` 全绿。
...
- 2026-09-09 修复验收（RESOLVED）：后端在 `build_entities` 中通过 `dual_run_result` CTE 聚合派生真实单位级凭证率；`dashboard.py` 金标异常恪守诚实性纪律如实上报 `None`，前端未知态优雅降级；`IssuesView` 明确“单位指标态现场判定”与“工单处置流转”分工；`CockpitTopBar` 调整为“实时集成脉搏”并采用 sky 色调区隔真库今日增量。全套单元测试与端到端编译全部通过。
