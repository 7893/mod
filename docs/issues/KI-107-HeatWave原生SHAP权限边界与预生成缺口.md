...
8. Owner 授权的首轮生产生成于 21:44:42 至 23:25:24 完成，实际用时约 100 分 42 秒；3,207 行、
   321 批全部发布，归因无效、特征指纹错配、模型时间戳错配均为 0。候选/输入/输出工作表、锁与临时
   脚本均已清理；API 抽查返回 `HEATWAVE_SHAP`，`mod_readonly` 仍只有 5 条只读授权。任务期间按设计
   延后的大屏快照已恢复为 `live/ok`。
9. 2026-09-24 00:00 定时重训于 01:46:11 成功结束，3,208 行新版成为 current，首轮 3,207 行完整
   成为 previous；训练、评分、321 批 SHAP、元数据持久化均成功，工作表与锁清理完成。最终验收发现
   新版解释表保存训练时间 `00:09:11.314815`，而 `ml_model_metadata.trained_at` 的秒精度值为
   `00:09:11`，API 因精度差异拒绝全部 3,208 行并降级为 `RULE_BASED`。本地已将生成时间截断到秒并
   将读取比较规范化到秒，26 项定向测试通过；生产库无需补写，待发布代码后即可接受现有快照。

10. 2026-09-24 时间精度修复已随签名提交 `37ef6ff` 经 Actions `35917950401` 部署为
    `20260923-204629`。按秒核对全表模型时间戳错配为 0，单位 1、1600 均返回 `HEATWAVE_SHAP`；
    大屏 `live/ok`、HeatWave 12/12、模拟器新鲜 SUCCESS、重训结果 success 与 3 小时超时均正常。
    API 账号仍为 USAGE 加四项 SELECT，无写入或 EXECUTE 权限；本次未补写数据库或重复生成。
...
- [x] 经数据库变更闸门授权后，在生产创建结果表并完成首轮生成；
- [x] 验证 API 返回 `HEATWAVE_SHAP`，API 账号权限仍为只读；
- [x] 下一次成功生成后验证本轮 3,207 行快照成为可恢复的上一版；
- [x] 完成定时重训、第二版全量生成与上一版保留及完整性验收；
- [x] 发布时间精度修复，确认现有快照恢复原生 SHAP 读路径并关闭 KI。
...
- 当前状态：代码与 3 小时 systemd 配置已部署；探针、首轮生成和首次定时闭环均已执行并清理；
  `ml_risk_explanation` 为 3,208 行新版，`ml_risk_explanation_previous` 为 3,207 行首轮可恢复快照。
  时间精度修复已部署，生产 API 抽查恢复 `HEATWAVE_SHAP`。
- 恢复验证边界：已确认上一版数据与时间戳完整保留，未在生产执行反向切换演练。上一版解释不等同于
  模型和特征的完整备份；单独恢复旧解释后若与当前模型或特征不匹配，API 仍会降级。
