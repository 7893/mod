...
- **全场景秒级响应 SLA 与台账筛选分页死锁治理（KI-059）**：
  ...
  - **动画绘制卫生**：大屏动画以 GPU 合成友好写法为基线（`AnimatedProgress` 用 `transform: scaleX`、`AnimatedNumber` 用 `requestAnimationFrame`+transform，ECharts 用一次性入场 `calmAnimation`）。已移除 `foundation.css` 中无任何引用的死代码 `@keyframes risk-breathe` 及仅服务该动画的孤儿变量 `--c-risk-glow`（`--c-risk-glow-soft` 仍在用，保留）。
