...
- **顶栏导航与状态区防重叠与断点优化**：在 `shell.css` 中为 `.header-nav` 设置 `flex-shrink: 0` 阻止导航选项被挤压而向外溢出；将顶栏按真实视口收缩断点由 `1600px` 调整为 `1880px`，彻底解决 1620px～1860px 视口下“风险预警”选中背景胶囊及风险数角标与右侧“实时数据/刷新受阻”状态徽标视觉交叉重叠的问题，全分辨率梯度安全间隙保持在 $\ge 10\text{px}$。
