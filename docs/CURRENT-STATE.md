...
- **全场景秒级响应 SLA 与台账筛选分页死锁治理（KI-059）**：
  ...
  - **SWR 生产退化（KI-061，OPEN）**：2026-09-08 只读复核发现日期锚点查询 `MAX(DATE(submit_time))` 在约 587 万行单据表上走 RAPID 全表扫描，后台预热长期不返回，API 持续提供缺少 `rolloutTrend`、`operationsTrend` 与双轨明细字段的旧 fallback，导致 C3/D3/D6 空白；连接与 HeatWave 健康探针仍为 `ok`，不能代表快照新鲜可用。修复前不得继续把 SWR 的毫秒响应等同于实时数据已刷新。
