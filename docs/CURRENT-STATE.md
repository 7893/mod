...
2026-09-26 workflow 恢复补充（取代下方手动部署要求）：用户要求沿用原有 workflow，
恢复 `main` push 或手动运行 Quality gates 后、检查通过即执行部署作业。
环境与依赖修复保持不变；本条记录触发规则恢复，实际部署结果以 Actions 为准。

2026-09-26 环境与检查发布补充（取代下文旧开发版本及 push 自动部署描述）：
本地与公开配置统一采用 Python 3.13.15、Node.js 26.10.0、pnpm 12.6.0、uv 0.12.5；
Python 约束、锁文件、Docker 构建配置与 CI 同步。push/PR 仅运行检查；
生产部署须手动运行 Quality gates 并显式选择 deploy。本项未部署生产。
文档检查调用隔离 Git hook 继承的仓库路径，保证 linked worktree 中也能解析 Lychee 固定版本。

## 2026-09-25 Python 工具链对齐 jpa（历史记录，发布状态见上方 2026-09-26 补充）

- Python 开发版本与 CI 固定为 3.13.15，项目要求为 `>=3.13.15,<3.14`，Ruff 目标为 `py313`。
- Docker Python 基础镜像对齐为 `python:3.13.15-slim`，uv 固定为 0.12.5；Node 26.10.0 / pnpm 12.6.0 保持不变。
- 本次仅修改本地配置，未修改远端主机运行时、未推送或部署；容器实际构建仍需另行验证。

## 2026-09-23 Node 26 与 pnpm 12 工具链统一（已部署）

- 2026-09-24 本地补充修复（尚未提交或部署）：Dockerfile 前端构建阶段从遗留的 Node 22 / pnpm 11
  对齐为 `node:26.10.0-alpine` / pnpm 12.6.0，使用 npm 显式安装 pnpm，不依赖镜像预装 Corepack。
  可移植性回归测试新增 Dockerfile 与 `.tool-versions` 的一致性断言。当前开发机未安装 Docker，
  完整容器构建尚未验证；既有生产发布状态不因本地修改而改变。
- 同日 Docker 静态复核补充（尚未提交或部署）：新增 `.dockerignore`，只纳入应用构建输入并排除
  本地依赖、环境配置、私有地图目录和构建产物；安装前复制 `pnpm-workspace.yaml`，保留 esbuild
  构建许可。容器移除 Debian 默认 Nginx 站点，将 MOD 配置声明为默认站点，并在镜像构建时执行
  `nginx -t`。已补充回归断言；按 Owner 要求本轮仅文件级复核，未运行测试或实际镜像构建。
- 随后 Owner 授权提交、推送与部署：提交前完整 `make check` 已通过（后端 284 项、前端 169 项、
  项目脚本 35 项，含 Docker 配置回归断言）。继续沿用现有 Actions 非容器发布流程，不安装 Docker，
  不构建或启动容器；镜像实际构建仍未验收。发布结果以本次提交的 Actions 记录和线上探针为准。
...
## 2026-09-23 HeatWave SHAP 预生成方案（2026-09-24 验收完成，KI-107 DONE）
...
- [KI-107](issues/KI-107-HeatWave原生SHAP权限边界与预生成缺口.md) 采用“管理员后台预计算、API 只读消费”路线：
  ...
  并保留上一版回滚快照。
- API 实现已移除请求内 `sys.ML_EXPLAIN_ROW` 调用，只读取 `mod.ml_risk_explanation`；
  只有特征指纹和模型训练时间同时匹配才标记 `HEATWAVE_SHAP`，快照缺失、无效或过期时仍诚实降级为
...
- 上述时间精度缺陷已于 2026-09-24 随签名提交 `37ef6ff` 经 Actions `35917950401` 修复部署，
  release 为 `20260923-204629`。全表按秒匹配错配为 0，API 抽查恢复 `HEATWAVE_SHAP`，大屏
  `live/ok`、HeatWave 12/12、模拟器与定时任务正常；账号保持只读。KI-107 已关闭，previous 的
  3,207 行保留及完整性已核验，未进行生产反向切换演练，也不代表旧模型与旧特征的完整恢复。
...
- 2026-09-11 恢复 USA 生产部署机与 JPA 专属开发机职责分离架构（ADR-0012）：
  ...
  - **USA（纯生产部署机）**：承载生产运行环境，核心服务经轻量守护器统一归并为单一 `mod.service`（由 `scripts/project/run_unified.py` 同时拉起与托管 FastAPI 8100 端口与 `mod-simulator` 仿真引擎，支持一键热重载与崩溃自愈）与 MODO `modo.service`（统一托管 8000 端口与 `modo-ingest` 节点打卡），同子网局域网连接 US MySQL HeatWave（`10.0.x.x (内网地址，已脱敏)`，< 0.2ms 极速延迟），API 隔离使用 `mod_readonly` 账号。
- 访客入口经 **AWS CloudFront** 前置（隐藏源站，见下条"源站隐藏架构"）；DNS 托管在 **Google Cloud DNS**，
  站点主机名以指向 CloudFront 分发的 CNAME 记录对外解析，DNS 层查不到源站真实 IP。同时支持生产域名无缝跳转访问。具体域名、分发 ID、
  回源地址等见部署配置，不写入文档。
