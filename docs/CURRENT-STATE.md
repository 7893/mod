...
- **2026-09-21 A 屏标题与实时数字微调（本地待提交）**：现行架构已退役 A3，第二块左栏面板编号为 A4；A2/A4 改为短标题常显、完整说明在整条标题栏悬停展示。A6 数字矩阵已居中并接入共用缓动数字；SSE 已提交事件只叠加晚于当前权威快照的单据/凭证增量，快照时间追上后自动撤销临时叠加，避免实时感缺失或重复计数。
