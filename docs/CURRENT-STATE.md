...
- 每日指挥部决策简报（KI-034 第二期）：后台服务 `mod-daily-briefing.timer`（HKT 00:30 触发）调用
  ...
  点击进 F 屏；接口 `GET /api/insights/briefing`。LLM 降级时不写假简报。低频日报不接收尚未封账的
  当日单据/凭证新增量，首页公开简报生成时点；实时日内量继续由权威快照与实时投影负责。
...
- **2026-09-21 审批/制证拟真链路与 D7（已部署）**：D6 收窄后与 D7 按内容密度采用 7:5，D6 内部核验内容与
  5:7 结构不变，D7 以 2×3 事实矩阵展示新数据的待审核、待制证、今日驳回、今日已制证、平均审核/制证时长。
  ...
  详见 [ADR-0020](decisions/0020-新业务采用分阶段审批制证链路.md)。D6/D7 的 7:5 信息密度再平衡随
  `54b1ce3` 部署，原 5:5 仅代表首次上线形态。
- **2026-09-21 AI 简报开放日口径修复（已部署）**：每日 00:30 低频简报不再向模型传入仍会增长的
  当日单据/凭证新增量，并禁止把累计规模改写为今日新增；首页公开简报生成时点，历史已入库文本不回写。
  提交 `54b1ce3` 的质量门与 Deploy 均成功，线上 `generatedAt` 已返回带 UTC 偏移的无歧义时间。
