...
- **2026-09-21 审批/制证拟真链路与 D7（本地待提交）**：D6 收窄为第三行左半区且内部核验内容与 5:7
  结构不变，右半区新增 D7 展示新数据的待审核、待制证、今日驳回、今日已制证、平均审核/制证时长。
  新模拟单据不再同周期机械生成一张凭证，而是经过工作日/节假日容量、稳定长尾时延与金额相关驳回阀门；
  制证支持 1:1、同单位 N:1 合并和少量 1:N 拆分。新链路用 `正式业务·拟真审批链` 隔离，历史数据库
  不回填、不重算；单据/凭证/集成按各自真实发生日期更新 `daily_stats`，详见 [ADR-0020](decisions/0020-新业务采用分阶段审批制证链路.md)。
