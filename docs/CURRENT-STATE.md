...
- HeatWave 内存加速看门狗与自愈落地（KI-049）：应用内置轻量看门狗模块（`app/heatwave_watchdog.py`）提供 ~1ms 级状态探测与缺失表 `SECONDARY_LOAD` 自动自愈；FastAPI `lifespan` 启动生命周期接入开机自检与自动补载；`/api/health` 探针透出 `heatwave: {status, loaded_count, total_target, loaded_tables, missing_tables}`，消除加速可观测盲区与静默降级风险；CLI 运维工具扩展 `watchdog` 指令支持周期巡检；新增自动化回归测试（`test_heatwave_watchdog.py`，全量 194 项后端单测全绿）。
