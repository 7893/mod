...
- **2026-09-20 Token 与物料化治理（本地重构，未部署）**：ECharts 字号已收敛到 `charts/tokens.ts` 的 `CHART_FONT`，既有样式检查器阻断新增裸 `fontSize`；A4 趋势图、A1 顶栏及 B/C/D/E/F 屏图表已完成 option 纯函数化和 `ChartCanvas` 迁移，统一画布补齐加载、错误、空态、autoresize 与语义点击透传。KI-099/100 保持 OPEN，待 A 屏剩余面板及特殊图表渐进迁移。
