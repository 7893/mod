...
- **2026-09-21 A5 当日指标同口径修复（本地待提交）**：今日单据与今日凭证统一读取同一条 `daily_stats` 业务日记录，A5 标题栏明确显示统计日期；D2 当天趋势复用该记录，避免模拟器持续写入时同一响应出现数笔漂移。省域单据增量同步改为快照业务日，不再把前一完整日单据与当日凭证并列为“今日”。
